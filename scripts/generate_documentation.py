...
def gen_doc(target_layers: Optional[List[str]] = None) -> None:
    ...
    try:
        spark = sps.create_spark_session()
    except Exception as e:
        spark = None
        print(f"Spark Session could not be created: {e}")
        print("Please insert a csv schema in the following path: ./outputs/schemas/")
        print("A script will be generated for you to run into you spark Session instead.")
        print("Please look at it under: ./output/schema/")
    ...
    with open(f"{TEMPLATE_DIR}/etl_base_html.txt", 'r', encoding="utf-8") as f:
        etlbasehtml = f.read()
    with open(f"etl_base.py", 'r', encoding="utf-8") as f:
        etlbasecode = f.read()
    ...
    # Generate schema script if needed
    if spark is None:
